...
for i in range(20):
    ...
    all_glo_obj=[]
    all_t=[]
    sat_players_set=intial_(sat_players_set, 1)
    ...
    while True:
        ...
        a_player = random.choice(sat_players_set)  #randomly select a player, update asynchronously
        
        #Best Response
        temp_act = a_player['action_set']
        Best_action = [temp_act[0]]
        Best_uti = cal_local_uti(C, D, a_player['id'], a_player['feasible_tasks'], a_player['neighbor_set'],temp_act[0], sat_players_set)
        for a in temp_act[1:]:
            uti = cal_local_uti(C, D, a_player['id'], a_player['feasible_tasks'], a_player['neighbor_set'], a, sat_players_set)
            if uti >  Best_uti:
                Best_action = [a]
                Best_uti = uti
            elif uti == Best_uti:
                Best_action.append(a)
        new_action = random.choice(Best_action)
        a_player['action']=new_action
...
#np.save("C:/Users/am is are/Desktop/draft_data/BR_F",F_record)  #保存全局目标函数值变量  
#plt.savefig('C:/Users/am is are/Desktop/draft_figure/BR_F50000.pdf',dpi=1200,bbox_inches='tight')
